...
int Crash(My *my_status_pointer, Enemy *Enemy_list, int board[SIZE][SIZE], doubleLayer layers);
void DrawChara(My *my_status_pointer, int board[SIZE][SIZE], Enemy *Enemy_list, doubleLayer *layers);
void Draw_Right(My *my_status_pointer, doubleLayer *layers);
...
int BFS(int board[SIZE][SIZE], Enemy *enemy_status_pointer, int player_x, int player_y, int move_list[2])
{
  ...
  while (1)
  {
    x = Stack[search_count].x;
    y = Stack[search_count].y;
    if (board[x - 1][y] != 1)
    {
    ...
    if (my_x + r >= e_x - r && my_x - r <= +e_x + r)
    {
      if (my_y + r >= e_y - r && my_y - r <= +e_y + r)
      {
        ...
        else if (Enemy_list[i].nervus == 1)
        {
          ...
          Draw_Right(my_status_pointer, &layers);
  ...
  while (1)
  {
    ...
    if (my_status_pointer->dead == 0)
    {
      ...
      if (wait % WRITELOAD == 0)
      {
        ...
        //衝突判定
        Crash(my_status_pointer, Enemy_list, board, layers);
      }
    ...
    else //パックマンが死んだときの処理
    {
      ...
      if (Packman_Dead(my_status_pointer, &layers) == 1)
      {
        ...
        HgBoxFill(WINDOW / 4 - 7, WINDOW / 2, 400, 85, 0);
        HgSetFont(HG_HB, 70);
        HgSetColor(HG_BLUE);
